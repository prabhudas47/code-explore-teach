...
export const CertificationsSection = () => {
  ...
  const { data: certCategories } = usePortfolioData('certifications', defaultCerts);
    ...
    <section className="py-24 sm:py-32 relative">
      <div className="max-w-6xl mx-auto px-4 sm:px-6" ref={ref}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {certCategories.map((cat: any, i: number) => (
            <div key={i} className={`glass-card rounded-xl p-6 text-center transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} style={{ transitionDelay: `${300 + i * 120}ms` }}>
