...
const defaultSkills = {
  left: ['Python', 'SQL', 'Data Structures', 'Machine Learning Foundations'],
  right: ['n8n Automation', 'API Integrations', 'Data Analysis', 'Technical Documentation'],
};

export const SkillsSection = () => {
  const { ref, isVisible } = useScrollReveal();
  const { data: skills } = usePortfolioData('skills', defaultSkills);
    ...
    <section id="skills" className="py-24 sm:py-32 relative">
      <div className="max-w-6xl mx-auto px-6" ref={ref}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-20 gap-y-0">
          {[skills.left, skills.right].map((col: string[], ci: number) => (
            <div key={ci} className="space-y-0">
              {col.map((skill: string, si: number) => (
                <div key={skill} className={`group border-b border-border py-6 transition-all duration-600 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`} style={{ transitionDelay: `${200 + (ci * 4 + si) * 100}ms` }}>
