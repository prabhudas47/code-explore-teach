...
const defaultTimeline = [
  { period: '2023 — 2027', title: 'B.Tech Computer Science (Data Science)', institution: 'NRI Institute of Technology' },
  { period: '2021 — 2023', title: 'Intermediate', institution: 'Sri Chaitanya Junior College' },
];

export const JourneySection = () => {
  const { ref, isVisible } = useScrollReveal();
  const { data: timeline } = usePortfolioData('education', defaultTimeline);
        ...
        <div className="relative">
          ...
          <div className="space-y-16">
            {timeline.map((item: any, i: number) => (
              <div key={i} className={`group relative pl-10 transition-all duration-700 cursor-default ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transitionDelay: `${300 + i * 200}ms` }}>
